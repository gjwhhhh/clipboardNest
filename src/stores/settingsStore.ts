...
const defaultHotkey = /Mac|iPhone|iPad|iPod/.test(navigator.platform)
  ? "Cmd+Shift+V"
  : "Ctrl+Shift+V";
...
export const useSettingsStore = create<SettingsState>((set) => ({
  ...
  fetchSettings: async () => {
    ...
    try {
      ...
      const settings: Settings = {
        ...
        hotkey: raw.hotkey || defaultHotkey,
        launchAtLogin: raw.launch_at_login === "true",
  ...
  updateHotkey: async (hotkey) => {
    try {
      await api.updateHotkey(hotkey);
      set((state) => ({
        settings: { ...state.settings, hotkey },
        error: null,
      }));
    } catch (error) {
      set({ error: String(error) });
